<template>
  <div class="contract-viewer">
    <ContractView @compare="compare" :contract="contract" :versions="versions" v-on:changeVersion="changeVersion"></ContractView>
    <div class="compare-viewer" v-show="compareView">
      <div class="diff-toobar">
        <span  :style="{margin: '5px'}" v-if="info">{{info.contract}}({{info.version}}-{{info.compareVersion}})</span>
        <button :style="{margin: '5px'}" @click="compareView=false">关闭</button>
      </div>
      <div class="diff-viewer">
        <code-diff :old-string="rightJson" :new-string="leftJson" :context="10" />
      </div>
    </div>
  </div>
</template>
...
export default class Viewer extends Vue {
  ...
  async compare(info:  {
    ...
    this.info = info
    let ctx = await getFactory().getContextFactory().context(info.contract, info.compareVersion).then(res => {
      return res.contract()
    });
