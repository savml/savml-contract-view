  ...
  <div class="contract-view" v-if="!!contract">
    <div class="contract-nav" v-show="!hideNav">
      ...
      <ul class="menu-root">
        ...
        <!-- 包依赖 -->
        <li v-if="contract.dependencies.length"><h3 class="nav-title">包依赖 <i>({{contract.dependencies.length}})</i></h3></li>
        <ul class="menu-sub">
          <template v-for="(item) in contract.dependencies">
            <li :key="item.package" class="menu-item">
              <a v-bind:href="'/' + item.package + '/' + item.version" class="sidebar-link">
        ...
        <!-- 服务 -->
        <li v-if="contract.services.length"><h3 class="nav-title">服务列表 <i>({{contract.services.length}})</i></h3></li>
        <ul class="menu-sub">
        ...
        <!-- 结构 -->
        <li v-if="contract.structs.length"><h3 class="nav-title">数据结构 <i>({{contract.structs.length}})</i></h3></li>
        <ul class="menu-sub">
        ...
        <!-- 视图 -->
        <li v-if="contract.pages.length"><h3 class="nav-title">页面视图 <i>({{contract.pages.length}})</i></h3></li>
        <ul class="menu-sub">
        ...
        <!-- 错误代码 -->
        <li v-if="codes.length"><h3 class="nav-title">错误代码 <i>({{codes.length}})</i></h3></li>
        <ul class="menu-sub">
          <template v-for="(item) in codes">
            <li :key="item.code" class="menu-item">
              <a v-bind:href="'#codes.' + item.code" class="sidebar-link" v-bind:title="item.cn || item.en">
                {{ item.code }}
              </a>
            </li>
          </template>
    ...
    height: 30px;
    border-radius: 50px;" @click="hideNav = !hideNav" title="显示隐藏侧边栏">
        {{hideNav ? '&gt;': '&lt;'}}
      </button>
    ...
    border: 0;" @click="scrollTop">⬆️</button>
      <!-- 服务列表 -->
                  ...
                  <tr v-for="action in service.actions" :key="action.name">
                    ...
                    <td>
                      <div v-for="code in (action.codes || [])" :key="code.code">
                        <a v-bind:href="'#codes.' + code.code" >{{code.code}}</a>
                      </div>
                    </td>
                  </tr>
                </table>
        ...
        <template v-for="struct in contract.structs" >
          <section class="section-detail" :key='struct.name' v-bind:id="'structs.'+ struct.name">
            ...
            <dl class="kv-item">  
              <dt>字段</dt>  
              <dd>
                <table class="table" v-if="struct.fields && struct.fields.length">
                  <tr>
                    <th width="250">名称</th>
                    <th width="250">类型</th>
                    <th width="50">必须</th>
...
export default class ContractView extends Vue {
  ...
  getActionPath(action: any, service: any) {
    ...
    let actionName = action.name[0].toLowerCase() +  action.name.substr(1,  action.name.length)
    if (service.path) {
      return `${service.path}/${actionName}`
    }
    return '/' + service.name[0].toLowerCase() +  service.name.substr(1,  service.name.length) + '/' + actionName
  }
  toHash (hash: string) {
    location.hash = hash
  }
